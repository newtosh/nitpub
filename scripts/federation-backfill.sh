#!/usr/bin/env bash
# Deliver never-federated posts to followers (stable activity IDs; skips already-shared).
#
# Usage:
#   NITPUB_URL=https://blog.example.com NITPUB_USER=admin NITPUB_PASS='...' bash scripts/federation-backfill.sh
set -euo pipefail
...
resp="$(curl -sS -b "$cookie_jar" -X POST "${base}/api/admin/federation/backfill")"
echo "$resp" | jq .
