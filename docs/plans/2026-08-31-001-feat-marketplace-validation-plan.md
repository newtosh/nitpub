...
1. **DO Vendor Portal onboarding** — **submitted 2026-08-31** via `marketplace.digitalocean.com/vendors`'s application form, then their automated follow-up required a **second step**: a "Vendor Portal Registration" email to `one-clicks-team@digitalocean.com` (team emails/owner, DO Team name, company name) — **sent 2026-08-31**. DO's stated SLA: **up to 10 business days** to get the initial version live and approved. The DO account's Team was renamed from the default "My Team" to `nitpub` to match the vendor registration (was previously holding prod VPS resources under the unnamed default — reused rather than creating a second team, no functional downside for a solo maintainer). Once approved: submit U1's built snapshot (via U2's CI output) through `cloud.digitalocean.com/vendorportal`.
