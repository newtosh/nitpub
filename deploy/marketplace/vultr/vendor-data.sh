#!/usr/bin/env bash
# Copy of deploy/startup-do-vultr.sh, kept identical, submitted as the
# Vultr Marketplace "Build from Vendor Data" script (imageless path —
# no Packer/snapshot needed, per Vultr's own recommended convention).
# If this ever needs to diverge from deploy/startup-do-vultr.sh, document
# why right here instead of silently drifting.
#
